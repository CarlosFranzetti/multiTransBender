...
/* ============================================================
   RACKFORGE v0.4 — ALL THREE VIEWS
   1) ENGINE  — big spectrum. Click ON the line to add a band
      node there (max 6). Click between nodes to select a band.
      Drag white crossover handles. Full per-band controls.
   2) PANEL   — front hardware faceplates per device.
   3) RACK 3D — front + overhead 3D-ish view, still usable.
   ============================================================ */
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, userSelect: "none", touchAction: "none", width: box + 6 }}>
      <div onPointerDown={down} style={{ position: "relative", width: box, height: box, cursor: "ns-resize" }}>
        ...
        <div style={{ position: "absolute", left: pad - 3, top: pad - 3, width: size + 6, height: size + 6, borderRadius: "50%", background: "radial-gradient(circle at 50% 35%, rgba(0,0,0,.15), rgba(0,0,0,.55))", boxShadow: "0 5px 12px rgba(0,0,0,.55)" }} />
        <div style={{ position: "absolute", left: pad, top: pad, width: size, height: size, borderRadius: "50%", ...body, boxShadow: "inset 0 1px 2px rgba(255,255,255,.28), inset 0 -3px 6px rgba(0,0,0,.45)", transform: `rotate(${ang}deg)` }}>
          {variant === "chicken" ? (
...
function TopPlate({ device }) {
  ...
  return (
    <div style={{
      ...
      {/* tubes glowing through vents */}
      {Array.from({ length: device.tubes }).map((_, i) => (
        <div key={i} style={{
          position: "absolute", left: `${14 + i * 11}%`, top: "30%", width: 26, height: 46, borderRadius: "50% 50% 40% 40%",
          background: "radial-gradient(circle at 50% 65%, #ffb75e, #e0632e 55%, rgba(120,30,10,.6))",
          boxShadow: "0 0 26px rgba(255,140,60,.75), 0 0 60px rgba(255,120,40,.35)",
          opacity: 0.9,
        }} />
      ))}
      {/* transformer / psu block */}
      <div style={{ position: "absolute", right: "7%", top: "18%", width: "20%", height: "52%", background: "linear-gradient(180deg,#26262a,#141416)", borderRadius: 5, boxShadow: "0 6px 14px rgba(0,0,0,.5), inset 0 1px 0 rgba(255,255,255,.1)" }}>
      ...
      <div style={{ position: "absolute", left: "8%", bottom: "6%", fontFamily: FONT_MONO, fontSize: 9, letterSpacing: 2, color: dark ? "rgba(255,255,255,.35)" : "rgba(0,0,0,.4)" }}>
        {device.name} · SERIAL RF-{device.id.toUpperCase().slice(0, 4)}-0001
      </div>
    </div>
  );
}
...
function Rack3D({ device, values, setValue, switches, setSwitch }) {
  return (
    <div style={{ padding: "6px 0 0" }}>
      ...
      {/* rack shadow floor */}
      <div style={{ height: 26, margin: "0 40px", background: "radial-gradient(60% 100% at 50% 0%, rgba(0,0,0,.5), transparent 70%)" }} />
    </div>
  );
}
...
function SpectrumDisplay({ crossovers, bands, selected, onSelect, onAddAt, onMoveXover, maxBands }) {
  ...
  return (
    <div style={{ borderRadius: 10, overflow: "hidden", boxShadow: "inset 0 2px 10px rgba(0,0,0,.7)" }}>
      <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} style={{ display: "block", width: "100%", background: "linear-gradient(180deg,#06080b,#0c0f14)", cursor: "crosshair" }} onClick={handleClick}>
        ...
        {[0.25, 0.5, 0.75].map((t) => <line key={t} x1={PADL} x2={W - PADR} y1={PADT + PH * t} y2={PADT + PH * t} stroke="rgba(140,160,190,.05)" />)}
        ...
        {bands.map((b, i) => {
          ...
          return (
            <g key={i}>
              <rect x={x0} y={PADT} width={x1 - x0} height={PH} fill={col} opacity={b.on ? (sel ? 0.15 : 0.06) : 0.02} />
        ...
        <path d={SPEC.line} fill="none" stroke="rgba(141,185,232,.6)" strokeWidth="1.7" pointerEvents="none" />

        {/* BAND NODES on the line at band centers */}
        {bands.map((b, i) => {
          ...
          return (
            <g key={`n${i}`} onClick={(e) => { e.stopPropagation(); onSelect(i); }} style={{ cursor: "pointer" }}>
              ...
              <circle cx={fToX(fc)} cy={specY(fc)} r={sel ? 8.5 : 6.5}
                fill={b.on ? col : "#33363d"} stroke="#0b0d10" strokeWidth="2"
                style={sel ? { filter: `drop-shadow(0 0 8px ${col})` } : undefined} />
              <text x={fToX(fc)} y={specY(fc) + 3.5} fill="#0b0d10" fontSize="9" fontWeight="800" fontFamily={FONT_LABEL} textAnchor="middle">{i + 1}</text>
            </g>
          );
        })}
        ...
        {crossovers.map((c, i) => {
          ...
          return (
            <g key={i} onPointerDown={startDrag(i)} style={{ cursor: "ew-resize" }}>
              <line x1={x} x2={x} y1={PADT} y2={PADT + PH} stroke="#e8e4d8" strokeWidth="1.3" opacity="0.8" />
              ...
              <rect x={x - 4.5} y={PADT + 2} width={9} height={9} rx={2} fill="#e8e4d8" />
            </g>
          );
        })}

        <text x={W - PADR - 4} y={PADT + 14} fill="#3f4855" fontSize="8.5" fontFamily={FONT_MONO} textAnchor="end">
          {bands.length < maxBands ? "click ON the line → new band" : "max 6 bands"} · click region/node → select · drag handles
        </text>
      </svg>
    </div>
  );
...
function EnvelopeMini({ attack, sustain, color }) {
  ...
  return (
    <svg width="230" height="72" style={{ display: "block", background: "linear-gradient(180deg,#07090c,#0c0f14)", borderRadius: 8, boxShadow: "inset 0 2px 6px rgba(0,0,0,.7)" }}>
      {[18, 36, 54].map((y) => <line key={y} x1="0" x2="230" y1={y} y2={y} stroke="rgba(140,160,190,.06)" />)}
...
function BandPanel({ band, index, crossovers, update }) {
  ...
  const gl = (t, c) => <div style={{ fontFamily: FONT_LABEL, fontSize: 9, letterSpacing: 2.5, color: c || "#6d7684", fontWeight: 800, marginBottom: 8 }}>{t}</div>;
  return (
    <div style={{ background: "linear-gradient(180deg,#191c22,#131519)", borderRadius: 10, padding: "14px 18px", border: `1px solid ${col}44`, borderTop: `3px solid ${col}`, boxShadow: `0 10px 24px rgba(0,0,0,.45), 0 0 30px ${col}12` }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
        <div style={{ fontFamily: FONT_LABEL, fontWeight: 800, fontSize: 16, letterSpacing: 2, color: col }}>BAND {index + 1}</div>
        <div style={{ fontFamily: FONT_MONO, fontSize: 9, color: "#6b7480" }}>{fmtHz(edges[index])}–{fmtHz(edges[index + 1])} Hz · independent transient + saturation chain</div>
      </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginRight: 6 }}>
              <select value={band.device} onChange={(e) => update({ ...band, device: e.target.value })} style={{ background: "#181b21", color: "#e6e2d6", border: `1px solid ${dev.accent}55`, borderRadius: 5, padding: "7px 9px", fontFamily: FONT_LABEL, fontSize: 12, minWidth: 150, boxShadow: "inset 0 1px 3px rgba(0,0,0,.5)" }}>
                {DEVICES.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
              </select>
              <div style={{ fontFamily: FONT_MONO, fontSize: 8, color: dev.accent, opacity: 0.8 }}>{dev.sub.toLowerCase()}</div>
            </div>
  ...
  return (
    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
      {bands.map((b, i) => {
        const col = BAND_COLORS[i], sel = i === selected;
        return (
          <div key={i} onClick={() => onSelect(i)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", borderRadius: 6, cursor: "pointer", background: sel ? `linear-gradient(180deg, ${col}26, ${col}10)` : "#101216", border: `1px solid ${sel ? col : "#262a32"}`, opacity: b.on ? 1 : 0.55 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: FONT_MONO, fontSize: 8, color: "#5a6572" }}>{fmtHz(edges[i])}–{fmtHz(edges[i + 1])}</div>
            </div>
            ...
            {bands.length > 1 && <button onClick={(e) => { e.stopPropagation(); onRemove(i); }} style={{ ...chipBtn(false, "#e0473d"), padding: "2px 7px" }}>×</button>}
          </div>
        );
      })}
    </div>
...
const chipBtn = (on, color) => ({ fontFamily: FONT_LABEL, fontSize: 8.5, fontWeight: 800, letterSpacing: 1, padding: "3px 7px", borderRadius: 4, cursor: "pointer", background: on ? `linear-gradient(180deg, ${color}, ${shade(color, -35)})` : "#14161a", color: on ? "#0d0f12" : "#6b7480", border: `1px solid ${on ? color : "#2c3038"}` });

/* ---------------- PREFS ---------------- */
function PrefsBar({ prefs, setPrefs, bandCount }) {
  const seg = (opts, key) => (
    <div style={{ display: "flex", background: "#0d0f13", border: "1px solid #2a2e36", borderRadius: 5, overflow: "hidden" }}>
      {opts.map((o) => (
        <button key={o} onClick={() => setPrefs({ ...prefs, [key]: o })} style={{ fontFamily: FONT_MONO, fontSize: 9.5, padding: "6px 10px", cursor: "pointer", border: "none", background: prefs[key] === o ? "linear-gradient(180deg,#f2d868,#d9b53a)" : "transparent", color: prefs[key] === o ? "#14161a" : "#8a93a0", fontWeight: prefs[key] === o ? 700 : 400 }}>{o}</button>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", gap: 20, alignItems: "center", flexWrap: "wrap", padding: "10px 16px", background: "linear-gradient(180deg,#111318,#0c0e11)", borderRadius: 8, border: "1px solid #23262e" }}>
...
/* ---------------- APP ---------------- */
export default function RackForgeV4() {
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(155px,1fr))", gap: 9 }}>
      {DEVICES.map((d) => (
        <button key={d.id} onClick={() => setActiveDevice(d.id)} style={{
          ...
          background: d.id === activeDevice ? `linear-gradient(180deg, ${shade(d.face, 10)}, ${shade(d.face, -20)})` : "#12141a",
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "22px 14px", color: "#c8cdd6", background: "radial-gradient(1400px 700px at 50% -15%, #23262e, #0a0b0d 65%)" }}>
      <div style={{ maxWidth: 1060, margin: "0 auto", display: "flex", flexDirection: "column", gap: 13 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
            ...
            <div style={{ fontFamily: FONT_MONO, fontSize: 9, color: "#5a6572" }}>preview v0.4 · all views</div>
          </div>
          <div style={{ display: "flex", borderRadius: 6, overflow: "hidden", border: "1px solid #2b2f38" }}>
            {[["engine", "ENGINE"], ["panel", "PANEL"], ["rack3d", "RACK 3D"]].map(([id, t]) => (
              <button key={id} onClick={() => setView(id)} style={{
                fontFamily: FONT_LABEL, fontSize: 11, fontWeight: 800, letterSpacing: 2.5, padding: "9px 17px", border: "none", cursor: "pointer",
                background: view === id ? "linear-gradient(180deg,#f2d868,#d9b53a)" : "#12141a", color: view === id ? "#14161a" : "#8a93a0",
              }}>{t}</button>
            ))}
          </div>
        </div>
        ...
        {view === "engine" && (
          <>
            <SpectrumDisplay crossovers={crossovers} bands={bands} selected={selected} onSelect={setSelected} onAddAt={addAt} onMoveXover={moveXover} maxBands={MAX} />
            <BandChips bands={bands} crossovers={crossovers} selected={selected} onSelect={setSelected}
              onToggle={(i) => setBands((s) => s.map((b, j) => (j === i ? { ...b, on: !b.on } : b)))}
              onSolo={(i) => setBands((s) => s.map((b, j) => (j === i ? { ...b, solo: !b.solo } : b)))}
              onRemove={removeBand} />
            <BandPanel band={bands[selected]} index={selected} crossovers={crossovers} update={(b) => setBands((s) => s.map((x, j) => (j === selected ? b : x)))} />
          </>
        )}
        ...
        <div style={{ fontFamily: FONT_MONO, fontSize: 9, color: "#3f4550", textAlign: "center" }}>
          engine: click ON the line → add band (max 6) · click node/region → select · panel & rack 3d: pick a device below · knobs drag vertically
        </div>
      </div>
    </div>
  );
